...
// 核心修改：点击切换角色 → 跳转到对应登录页面
const handleLoginTypeChange = (command) => {
  if (command === 'user') {
    router.push('/login') // 用户登录
  } else if (command === 'merchant') {
    router.push('/merchant/login') // 商家登录
  } else if (command === 'admin') {
    router.push('/admin/login') // 管理员登录
  }
}
...
.input-icon {
  ...
  z-index: 1;
  font-size: 18px;
  pointer-events: none; /* 防止图标挡住输入框点击 */
}

/* 强制修改输入框内边距，拉开文字和图标的距离 */
.custom-input :deep(.el-input__wrapper) {
  padding-left: 44px !important; /* 左边给图标留足空间 */
  padding-right: 40px !important; /* 右边给密码按钮留足空间 */
  border-radius: 8px;
